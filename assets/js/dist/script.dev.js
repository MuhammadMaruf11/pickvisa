...
(function ($) {
  ...
  document.getElementById('test1').innerHTML = countries.map(function (user) {
    return "<div class='col-xl-3 country-name'>\n                 <a href=".concat(user.value.toLowerCase(), ".html class='country-flag'>\n                    <img src=", "https://cdn.jsdelivr.net/npm/svg-country-flags@1.2.10/svg/".concat(user.value.toLowerCase(), ".svg"), " alt='icon' />\n                    <h6 class='cName'> ").concat(user.text, "</h6>\n                </a>\n            </div>");
  }).join('');
  /*------------------------------------
    Country name search
  --------------------------------------*/
  ...
  var btsearch = {
    init: function init(search_field, searchable_elements, searchable_text_class) {
  ...
  if (jQuery(".destination-slider-active .swiper-container").length > 0) {
    var destinationSlider = new Swiper('.destination-slider-active .swiper-container', {
      // Optional parameters
      slidesPerView: 5,
      slidesPerColumn: 1,
      loop: true,
  ...
  $(document).click(function (e) {
    if (!$(e.target).hasClass("track-btn") && $(e.target).parents(".track-content").length === 0) {
      $(".track-content").hide();
    }
  });
  ...
  $(document).click(function (e) {
    if (!$(e.target).hasClass("classes-btn") && $(e.target).parents(".classes-content").length === 0) {
  ...
  $(".category-counts").on("click", ".plus, .minus", function () {
    // Get current quantity values
    var qty = $(this).closest(".category-counts").find(".qty");
    var val = parseFloat(qty.val());
    var max = parseFloat(qty.attr("max"));
    var min = parseFloat(qty.attr("min")); // get input value 

    var adult = parseFloat($('#adult').val());
    var children = parseFloat($('#children').val());
    var infants = parseFloat($('#infants').val());
    var total = adult + children + infants; // console.log(total)
